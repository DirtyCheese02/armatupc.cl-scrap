...
CATEGORY_URL_MAP = {
    "Headphones": "https://etchile.net/categorias/perifericos/audio-y-streaming/audifonos/",
    "Mouse": "https://etchile.net/categorias/perifericos/mouse-accesorios/mouse/",
    "Keyboard": "https://etchile.net/categorias/perifericos/teclados/",
    "Storage": "https://etchile.net/categorias/componentes-partes-y-piezas/almacenamiento/",
    "ExternalStorage": "https://etchile.net/categorias/productos/almacenamiento-y-drives/discos-externos/",
    "Monitor": "https://etchile.net/categorias/productos/monitores/",
    "CPUCooler_Liquid": "https://etchile.net/categorias/componentes-partes-y-piezas/refrigeracion/water-cooling/",
    "ThermalCompound": "https://etchile.net/categorias/componentes-partes-y-piezas/refrigeracion/pasta-termica/",
    "PowerSupply": "https://etchile.net/categorias/componentes-partes-y-piezas/psu-fuentes-de-poder/",
    "Case": "https://etchile.net/categorias/componentes-partes-y-piezas/gabinetes/",
    "Memory": "https://etchile.net/categorias/componentes-partes-y-piezas/memorias/",
    "CPU": "https://etchile.net/categorias/componentes-partes-y-piezas/procesadores/",
    "VideoCard": "https://etchile.net/categorias/componentes-partes-y-piezas/tarjetas-de-video/",
    "Motherboard": "https://etchile.net/categorias/componentes-partes-y-piezas/placas-madres/",
    "Webcam": "https://etchile.net/categorias/perifericos/audio-y-streaming/webcam-camaras/",
    "NetworkAdapter": "https://etchile.net/categorias/productos/redes-y-conectividad/adaptadores-wifi/",
    "CPUCooler_CaseFan": "https://etchile.net/categorias/componentes-partes-y-piezas/refrigeracion/ventiladores/"
}
    ...
    async with sem:
        ...
        try:
            ...
            for i in range(1, total_pages + 1):
                ...
                links = await page.query("//a[contains(@class,'woocommerce-LoopProduct-link')]", find_all=True)
...
async def getPagination(Tab):
    try:
        ...
        pagination = await Tab.query("//nav[@class='woocommerce-pagination']/ul/li", find_all=True)
        if not pagination:
            return 1
...
async def scrape_product_details(sem, browser, url, category_name):
    async with sem:
        ...
        try:
            ...
            try:
                price = await page.query("//div[@class='price-wrapper']/p/span[contains(@class,'woocommerce-Price-amount')]")
                price = await price.text
                price = price.replace("$","").replace(".","").strip()
            except:
                try:
                    price = await page.query("//div[@class='price-wrapper']/p/ins/span[contains(@class,'woocommerce-Price-amount')]")
                    price = await price.text
                    price = price.replace("$","").replace(".","").strip()
                except:
                    pass
            ...
            # Imagen
            try:
                image_element = await page.query("//div[contains(@class,'woocommerce-product-gallery__wrapper')]//img")
...
# 5. CONSTRUIR JSON PLANO (FORMATO SOLICITADO)
            ...
            # Guardar Json
            if found:
                with open(f"ScrapDB/Outputs/ETChile/ETC_{hashlib.md5(url.encode()).hexdigest()}.json", "w", encoding="utf-8") as f:
